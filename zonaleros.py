# -*- coding: utf-8 -*-
"""Extractor de enlaces de descarga de zona-leros.com.
...
def _lanzar(url, tiempo_max=50):
    """Lanza Chrome con el perfil real y devuelve (ws_url_cdp, error)."""
    if _ws is None:
        return None, "falta la librería websocket-client en el venv"
    if _chrome_corriendo():
        return None, ("Chrome está abierto: ciérralo y reintenta. "
                      "El descargador necesita usar tu perfil para pasar "
                      "la verificación de Cloudflare.")
    ruta = _ruta_chrome()
    if not ruta:
    ...
    _respaldar_cookies()   # red de seguridad por si algo sale mal
...
def extraer(url):
    ...
    try:
        ...
        # si Chrome murió a mitad, dilo claro: no es de la página
        if not _chrome_corriendo():
